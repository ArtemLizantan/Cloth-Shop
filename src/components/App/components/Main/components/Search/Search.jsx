...
export function Search() {
  ...
  useEffect(() => {
    let qProducts = products.filter((product) =>
  ...
  const cats = useMemo(
    () => getCategories(queryProducts),
    [queryProducts, getCategories]
  );
          ...
          {queryProducts.length > 0 ? (
            ...
            <div className="search__notification" key={0}>
              <div className="search__icon">
                <i className="ic_shocked"></i>
              </div>
              <div className="search__message">
                There are <b>no products</b> matching your request
              </div>
            </div>
          )}
        </div>
